...
class EP_grad:
    ...
    def get_params_gradient(self, y0, target, nn, network_params):
        ...
        N_params = sum(jax.tree_util.tree_leaves(jax.tree.map(jnp.size, network_params)))
        ...
        for k in range(1, N_data):
            dEdParams_free = jax.tree_util.tree_map(jnp.add, dEdParams_free, nn.params_derivative(free_equi[k,:], network_params))
            dEdParams_nudge = jax.tree_util.tree_map(jnp.add, dEdParams_nudge, nn.params_derivative(nudge_equi[k,:], network_params))
